odoo.define('smm_intecproof.purchase_report', function(require) {
	'use strict';
	...
	var PurchaseReport = AbstractAction.extend({
		...
		events: {
			...
			// 'click #date_chose': '_onCalendarIconClick',
...
//                self.apply_filter();
		...
		_onCalendarIconClick: function(ev) {
			console.log('Click en el calendario',ev);
			var $calendarInputGroup = $(ev.currentTarget);
			console.log('Doble click',$calendarInputGroup);
			var calendarOptions = {
			...
			console.log(calendarOptions,'Para ver si se generó evento')
		...
		print_pdf: function(e) {
			...
			self._rpc({
				...
				var action = {
					...
					'report_name': 'smm_intecproof.purchase_order_report',
					'report_file': 'smm_intecproof.purchase_order_report',
					'data': {
						'report_data': data
					},
					'context': {
		...
		apply_filter: function() {
            ...
            console.log('DESDE '),$("filter_data_selected.date_from")
			...
			if ($(".report_type").length) {
				console.log('ACCION SELECCIONADA ',$(".report_type")[1].value)
